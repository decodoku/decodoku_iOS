//
//  Tut3ViewController.m
//  Decodoku
//
//  Copyright © 2016 James R. Wootton.

#import "Tut3ViewController.h"

@interface Tut3ViewController (){
...
@implementation Tut3ViewController
...
- (void)addContent {
    ...
    UIImageView *tut1 = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tut6.png"]]; // initialize and set image
    tut1.frame = CGRectMake(x1, y1, wG, hG); //frame
    [self.view addSubview:tut1];
    
    
    UIImageView *tut2 = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tut7.png"]]; // initialize and set image
    ...
    UILabel *_text6 = [[UILabel alloc] init]; // initialize
    _text6.frame = CGRectMake(0.47 * wS, 0.1 * hS, 0.5*wS, 0.0); // frame
    _text6.text = [NSString stringWithFormat:NSLocalizedString(@"tuttext3a", nil), @(1000000)]; // text
    _text6.textColor = [UIColor blackColor]; //text colour
    _text6.font = [UIFont fontWithName:@"QuicksandBook-Regular" size:MAX(0.035 * wS,14)]; // text font
    [_text6 setTextAlignment:NSTextAlignmentLeft]; // text alignment
    _text6.numberOfLines = 0; // word wrap
    [_text6 sizeToFit]; // height in frame is set to zero, but set to fit text here
    [self.view addSubview:_text6]; // add to view
    
    UILabel *_text7 = [[UILabel alloc] init]; // initialize
    _text7.frame = CGRectMake(0.47 * wS, 0.5 * hS, 0.5*wS, 0.0); // frame
    _text7.text = [NSString stringWithFormat:NSLocalizedString(@"tuttext3b", nil), @(1000000)]; // text    _text7.textColor = [UIColor blackColor]; //text colour
    _text7.font = [UIFont fontWithName:@"QuicksandBook-Regular" size:MAX(0.035 * wS,14)]; // text font
    [_text7 setTextAlignment:NSTextAlignmentLeft]; // text alignment
    _text7.numberOfLines = 0; // word wrap
    [_text7 sizeToFit]; // height in frame is set to zero, but set to fit text here
    [self.view addSubview:_text7]; // add to view
...
- (IBAction)menuButton:(id)sender {
    ...
    [self performSegueWithIdentifier:@"fromTut3" sender:self];
