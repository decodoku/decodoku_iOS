//
//  Tut4ViewController.m
//  Decodoku
//
//  Copyright © 2016 James R. Wootton.

#import "Tut4ViewController.h"

@interface Tut4ViewController (){
...
@implementation Tut4ViewController
...
- (void)addContent {
    ...
    UIImageView *tut1 = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tut8.png"]]; // initialize and set image
    tut1.frame = CGRectMake(x1, y1, wG, hG); //frame
    [self.view addSubview:tut1];
    
    
    UIImageView *tut2 = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tut9.png"]]; // initialize and set image
    ...
    UILabel *_text8 = [[UILabel alloc] init]; // initialize
    _text8.frame = CGRectMake(0.47 * wS, 0.1 * hS, 0.5*wS, 0.0); // frame
    _text8.text = [NSString stringWithFormat:NSLocalizedString(@"tuttext4a", nil), @(1000000)]; // text
    _text8.textColor = [UIColor blackColor]; //text colour
    _text8.font = [UIFont fontWithName:@"QuicksandBook-Regular" size:MAX(0.035 * wS,14)]; // text font
    [_text8 setTextAlignment:NSTextAlignmentLeft]; // text alignment
    _text8.numberOfLines = 0; // word wrap
    [_text8 sizeToFit]; // height in frame is set to zero, but set to fit text here
    [self.view addSubview:_text8]; // add to view
    
    UILabel *_text9 = [[UILabel alloc] init]; // initialize
    _text9.frame = CGRectMake(0.47 * wS, 0.5 * hS, 0.5*wS, 0.0); // frame
    _text9.text = [NSString stringWithFormat:NSLocalizedString(@"tuttext4b", nil), @(1000000)]; // text
    _text9.textColor = [UIColor blackColor]; //text colour
    _text9.font = [UIFont fontWithName:@"QuicksandBook-Regular" size:MAX(0.035 * wS,14)]; // text font
    [_text9 setTextAlignment:NSTextAlignmentLeft]; // text alignment
    _text9.numberOfLines = 0; // word wrap
    [_text9 sizeToFit]; // height in frame is set to zero, but set to fit text here
    [self.view addSubview:_text9]; // add to view
...
- (IBAction)menuButton:(id)sender {
    ...
    [self performSegueWithIdentifier:@"fromTut4" sender:self];
